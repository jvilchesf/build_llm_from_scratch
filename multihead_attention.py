...
class MultiHeadAttention(nn.Module):
    def __init__(self,
        ...
        Args:
            ...
            num_heads: Define number of heads, it referes number of times the multihead attention context will be apply
        """
        ...
        self.W_keys = nn.Linear(dim_in, dim_out, bias=qkv_bias) # [256 x 128]
        self.W_values = nn.Linear(dim_in, dim_out, bias=qkv_bias) # [256 x 128]
    ...
    def forward(self,
        ...
        query = self.W_query(X)  # X = [8 x 4 x 256] | W_query = [256 x 128] => 8 x4 x 128
        key = self.W_keys(X)  # X = [8 x 4 x 256] | W_key = [256 x 128] => 8 x 4 x 128
        value = self.W_values(X)  # X = [8 x 4 x 256] | W_values = [256 x 128] => 8 x 4 x 128

        # Reshaping query and view matrices, including num_heads and head_dim
        query = query.view(batch_size, num_tokens, self.num_heads, self.head_dim)  # [8 x 4 x 8 x 16]
        key = key.view(batch_size, num_tokens, self.num_heads, self.head_dim) # [8 x 4 x 8 x 16]
        value = value.view(batch_size, num_tokens, self.num_heads, self.head_dim) # [8 x 8 x 4 x 16]

        # Reshape from (batch x num_tokens x num_heads x head_dim) to (batch_size x num_heads x num_tokens x head_dim)
        ...
        attention_scores = query @ key.transpose(2, 3)  # [8 x 8 x 4 x 16] @ [8 x 8 x 16 x 4] = [8 x 8 x 4 x 4]
        ...
        # Context Vector
        context_vector = (attention_weights @ value).transpose(1, 2) # [8 x 8 x 4 x 4] @ [8 x 8 x 4 x 16] = [8 x 8 x 4 x 16].transpose(1, 2) -> [8 x 4 x 8 x 16]
        context_vector = context_vector.contiguous().view(batch_size, num_tokens, self.dim_out)  # [8 x 4 x 128] Reshaping to normal form to deliver output, combine heads, where self.dim_out = self.num_heads * self.head_dim
